...
*/
	if (m_bSensorInfoConfigured == true)
		return;
	else
	{ 
		short i, j;
	...
	do
	{
		if ((lResponse = ReadFile(m_ComPort, &cTemp[i], 1, &nBytesRead, NULL)))
		{
			...
			if (nByteCount > RESPONSE_LENGTH)
				return (-1);
			Sleep(10L);  // introduced to prevent hangs on return data.... 
		}
		else
			return lResponse;
	} while (bDone == FALSE);
...
int  CPRISpectro::GetApertureList()
{
	...
	int j;
	SendGetCommand(PRI_GET_APERTURE_INFO, outBuffer, LONG_TIMEOUT);
	...
	m_ApertureList.iApertureCount = j;

	return(m_ApertureList.iApertureCount);

}

int  CPRISpectro::GetBandWidthList()
{
	...
	int   iResponseLength, iCount, iOffset;
	int j;
	SendGetCommand(PRI_GET_BANDWIDTH_LIST, outBuffer, LONG_TIMEOUT);
	//return data is in m_cResponse
	iResponseLength = strlen(m_cResponse);
	iCount = 0;
	mp_cBuffer = m_cResponse;// virtualize m_cResponse 
	...
	while (mp_cBuffer[0] != 0)
	{
		...
		mp_cBuffer += iOffset;
		iCount += iOffset;
		m_BandWidthList.iBwIndex[j] = atoi(outBuffer);
		iOffset = ParseToComma(mp_cBuffer, outBuffer); // get degree string 
		mp_cBuffer += iOffset;
		iCount += iOffset;
		m_BandWidthList.csBandWidthList[j] = outBuffer;
		iOffset = ParseToCR_LF(mp_cBuffer, outBuffer);
		mp_cBuffer += iOffset;
		iCount += iOffset;
		j++;
	}
	m_BandWidthList.iBwCount = j;
	SendGetCommand(PRI_GET_BANDWIDTH_LIST, outBuffer, LONG_TIMEOUT);

	return(m_BandWidthList.iBwCount );
...
// tbd add get current status .... 
bool   CPRISpectro::GetCurrentStatus()
...
<CaptureMode>, <SyncPeriod>, <bandwidth>CRLF
	...
	SendGetCommand(PRI_CURRENT_STATUS, m_cResponse, SHORT_TIMEOUT);
	
	char *inString = m_cResponse;
	short i;
	i = 0;
	while (m_cResponse[i] != 0x0d)
		i++;
	m_cResponse[i-1] = NULL;
	int iTemp;
	sscanf_s(inString, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d",
		&iTemp,
		&m_CurrentStatus.iPrimaryLens,
		&m_CurrentStatus.iAddon1,
		&m_CurrentStatus.iAddon2,
		&m_CurrentStatus.iAddon3,
		&m_CurrentStatus.iCurrentAperture,
		&m_CurrentStatus.iUnits,
		&m_CurrentStatus.iExposureMode,
		&m_CurrentStatus.iExposureTime,
		&m_CurrentStatus.iGain,
		&m_CurrentStatus.iCaptureMode,
		&m_CurrentStatus.iSyncPeriod,
		&m_CurrentStatus.iBandwidth
	);
	return (true);
};